...
import { SentryError } from './errors/sentry-error'
...
export class HttpClient {
  ...
  async fetch<T = unknown>(
    ...
    try {
      ...
      const tags = {
        section: 'API',
        side: isSSR ? 'server' : 'client',
      }

      // console.log('API Layer', url, responder, tags)

      throw new SentryError(`API Layer ${url}`, responder, tags)
    } catch (error) {
...
const fetchWrap = async (args: TFetchWrapArgs) => {
