...
const getAllInternalTimestampKeys = async (): Promise<IDBValidKey[] | null> => {
  try {
    ...
    const internalKeys = keys.filter((thisKey: IDBValidKey) =>
      isInternalExpiryTimestampKey(thisKey as string) ? thisKey : false,
    )
    ...
    Sentry.captureException(error, {
      tags: {
        section: `idb - getAllInternalTimestampKeys`,
      },
    })
    return null
  }
...
export const removeExpiredKeys = async () => {
  try {
    ...
    const p = keys.map(async (thisKey: IDBValidKey) => {
      return get(thisKey).then((value) => (Date.now() > (value as number) ? remove(thisKey) : null))
    })
